"""
Figure 5 (relaxed) state space for Traveling Ethiopia - Cartesian coordinates.
States and adjacency for world file and path planner.
Coordinate system: (x, y, z) in meters. Addis Ababa at origin.
"""

# Relaxed state space: key cities from Figure 5 with Cartesian coordinates (x, y, z)
# Format: "State Name": (x_m, y_m, z_m)
FIGURE5_STATE_COORDINATES = {
    "Addis Ababa": (0, 0, 0.5),
...
# Relaxed graph: adjacency (undirected). Subset of Figure 5 connections.
FIGURE5_GRAPH = {
...
def get_state_coordinates():
    """Return dict of state name -> (x, y, z)."""
    return FIGURE5_STATE_COORDINATES.copy()

def get_graph():
    """Return graph as adjacency dict (each edge once; add reverse for undirected)."""
    import copy
    g = copy.deepcopy(FIGURE5_GRAPH)
    for u, neighbors in list(FIGURE5_GRAPH.items()):
        for v in neighbors:
            if v not in g:
                g[v] = []
            if u not in g[v]:
